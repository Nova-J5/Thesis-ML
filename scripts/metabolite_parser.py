...
xml_spectra_path = "../spectra/hmdb_predicted_cms_spectra"


def get_spectrum(spectrum_id):
    xml_spectrum_file = os.path.join(xml_spectra_path, 'HMDB0000001_c_ms_spectrum_{}_predicted.xml'.format(spectrum_id))
    ...
    if os.path.exists(xml_spectrum_file):
        ...
        spectrum = {'id': spectrum_id, 'peak_counter': root.find('peak_counter').text,
                    'ionization_mode': root.find('ionization_mode').text}

        # get the peaks
        peaks = []
        for peak in root.findall('c-ms-peaks/c-ms-peak'):
            peaks.append({
                'mass-charge': peak.find('mass-charge').text,
    ...
    with open(output_file, 'w', newline='') as xmlfile:
        ...
        for metabolite in root.findall('metabolite'):
            ...
            for s in metabolite.findall('spectra/spectrum'):
                if find_element(s, 'type') != 'Specdb::CMs':
                    continue
                spectrum = get_spectrum(find_element(s, 'spectrum_id'))
                if spectrum:
                    valid_spectrum = spectrum
                    break  # interrompi il ciclo non appena trovi il primo spettro valido
            ...
            xmlfile.write('    <super_class>' + find_element(metabolite, 'taxonomy/super_class') + '</super_class>\n')
            ...
            for p in metabolite.findall('predicted_properties/property'):
                if find_element(p, 'kind') == 'logp':
                    ...
                    xmlfile.write('    <pka_strongest_acidic>' + find_element(p, 'value') + '</pka_strongest_acidic>\n')
                elif find_element(p, 'kind') == 'pka_strongest_basic':
                    xmlfile.write('    <pka_strongest_basic>' + find_element(p, 'value') + '</pka_strongest_basic>\n')
                    ...
                    xmlfile.write('    <rotatable_bond_count>' + find_element(p, 'value') + '</rotatable_bond_count>\n')
                    ...
                    xmlfile.write('    <physiological_charge>' + find_element(p, 'value') + '</physiological_charge>\n')
